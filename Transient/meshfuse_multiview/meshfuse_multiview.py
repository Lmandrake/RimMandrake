...
HERE = "/mnt/d/Luke/dev/Rimworld/Transient/meshfuse_multiview"
MESH = "/mnt/d/Luke/dev/Rimworld/Transient/meshfuse_multiview/instantmesh_multiview.obj"
...
def main():
    ...
    # AZ WINDOW: south/north are near-symmetric boxy silhouettes -- a free 0-360
    # global search collides them onto the SAME winning pose (proven: both hit
    # az=270,el=35 before this fix). Each view's camera is nominally fixed
    # (BASE_AZ); the gate should only correct SMALL pose error around that
    # nominal position, not roam the whole sphere and find a different view's
    # pose. Window +/-40deg (3x the 15deg-step global search's granularity)
    # comfortably covers real per-view camera bias while staying far short of
    # the 180deg gap to the opposite (ambiguous) view.
    SW=160
    gate={}
    AZ_WINDOW=40.0
    AZ_STEP=5.0
    ...
    for v in VIEWS:
        ...
        best=(-1,None,None)
        base=BASE_AZ[v]
        n_steps=int(round(2*AZ_WINDOW/AZ_STEP))
        az_grid=[(base-AZ_WINDOW+i*AZ_STEP)%360.0 for i in range(n_steps+1)]
        for az in az_grid:
            for el in el_grid:
    ...
    rows=["INPUT sprite","RAW InstantMesh (4-view)","FUSED mesh"]
    def fit(img,cell):
