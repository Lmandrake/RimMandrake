# DynamicLighting — scheme catalog, DRAFT

**2026-09-16 · RimMandrake tier (campaign-agnostic) · binds nothing.** A creative catalog for the
owner to react to. No defs, no code, no engine claims. Nothing else in the repo was touched to
write it. `Transient/` shelf life ~14 days — no committed doc may cite this path; if any of it is
kept, it moves to `design/RimMandrake/` first.

The mod: a framework + resource mod that takes the player's grid of colour-adjustable lights and
animates it as a tapestry. A campaign layer (the Utinni) rides on top, in which nine gods and the
Narrator speak through the ship's lights and the player never picks a scheme himself.
