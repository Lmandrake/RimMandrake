...
### A code-review pass marked two patch files CLEAN on the strength of their own header comment

**Symptom:** two patch files carried a header comment asserting *"MayRequire on
the Operation is load-bearing"*. A full-file review read the comment, agreed it
sounded right, and marked both CLEAN. The claim was false, and shipping it
killed a full-list load.
**Cause:** an engine claim written in a file's own header is **prose, not
evidence** — the same rule `SKILL.md` §1b states for docs applies to comments
inside the file being reviewed. Nothing forces a comment to stay true as the
file around it changes, and a reviewer reading the comment is reading the
author's belief, not the engine.
**Fix:** verify any engine claim in a header against decompiled source
(`rimsage`) before a review accepts it — the same standard applied to a design
doc's "engine status" section.
**Recurs when:** a file header states *why* something works, not just *what*
it does — "load-bearing", "required because", "the engine needs this" are the
tell. (2026-09-17)
