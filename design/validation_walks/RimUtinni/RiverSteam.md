...
## the walk
1. [L] Player.log after load contains no "Config error in mandrake.rm.flowworks" (the id this hook now ships under, since ManyWaters merged into FlowWorks) and no XML error naming RimMandrake.ManyWaters — load-time (this hook ships no XML of its own, so a clean load is the only load-time signal there is)
