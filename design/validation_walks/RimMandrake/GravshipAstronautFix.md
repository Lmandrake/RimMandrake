...
- `Things/Structures/GravshipGenebank/GravshipGenebank_north.png` must exist, 128x128, and must NOT be pixel-identical to
  <!-- walklint-ok: GravshipGenebank_south.png is the DONOR's (vanillaexpanded.gravship) own shipped file, not ours -- never present under src/ -->
  `GravshipGenebank_south.png` (the whole point — the donor's own unrotated south substitution would be).
...
## the walk
1. [L] Player.log after load contains no "Config error in mandrake.rm.patches" (the id this fix now ships under, since it absorbed into MandrakePatches) and no "Failed to find any textures at" naming `MechAncient_Astronaut` or `GravshipGenebank`   # load-time
