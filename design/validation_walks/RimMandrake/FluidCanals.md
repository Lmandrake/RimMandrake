...
- Step 4 asserts `volume` = 60 on the comp. The comp's real fields are `dripVolume`/`dripIntervalTicks`/`reFloodVolume`/`reFloodIntervalTicks`; `reFloodVolume` is the 60. UNMEASURED whether a bare `volume` field still exists on `CompProperties_FluidReservoir` — check before trusting that line.
