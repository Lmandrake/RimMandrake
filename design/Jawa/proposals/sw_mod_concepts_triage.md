...
| 4 | The Living Desert (burrowing, sand-swimming, thumpers, leviathans, krayt pearls, night desert, weather-with-teeth, glasslands) | **PARTIAL COVERED + LIVE → expanded** | the rest of this concept — burrowing ecology, sand-swimming, krayt leviathans and pearls, night desert — remains unbuilt per `FUTURE_VECTORS.md` ("pairs with the VAST-tier precedent already in-stack: the Leviathans sandworm, a world object with weather and music attached, not a spawn"). Expanded below, scoped narrowly around that gap. A "weather-with-teeth" mechanism has a design for it (`DUST_STORMS_DESTRUCTIVE_1`, a Tornado-derived `RM_DustDevil`) — that item is closed as a ruled spec 2026-08-31; no `RM_DustDevil` implementation has actually landed in the repo yet. |
...
| 22 | Rare Force Phenomena | **COVERED/PARKED** | `FORCE_SYSTEM_OWNERSHIP_1` (dropped 2026-08-30 — owner ruled lightsabers/Force stay upstream, outside the ingest plan, so its own-DLL question is moot) once raised whether the Force-*powers* sibling mod was even wanted; `FUTURE_VECTORS.md` separately parks "Force powers in their entirety (VPE returns in v2)." Not mine to invent ahead of a future reopening. |
...
| 27 | Jawa Cultural Flavor (Utinni! vocal/emote layer) | **COVERED-adjacent, one-liner** | `PAWN_FLAVOR_STARWARS_1` (closed 2026-08-30 — backstories/traits/text flavor shipped; follow-on prose continues under `PAWN_FLAVOR_PHASE2_PROSE_1`) owns backstories/traits/text flavor; the *audio* emote layer specifically is unclaimed but needs actual sound design, not a design doc — flagged, not expanded |
