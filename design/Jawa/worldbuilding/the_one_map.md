...
- ✅ **One planet, authored.** The map lives in the LIVE world, edited directly over
  the bridge (`ASHKARR_WORLD_DEFINITION.md` §12), and ships to players as the
  canonical start save (currently `CANONICAL_ASHKARR_START_2026-09-12.rws`).
  Judged on **realism first** — does it read as a photograph of a real world — then
  against the design docs below.
...
## 🔑 Where the map actually lives — reconciled 2026-09-18, owner ruling ("Rewrite to match reality"), worldmap docs pass

- ⭐ **THE MAP is the live authored world**, edited tile by tile over the bridge, and
  it ships to players as the canonical start save (currently
  `CANONICAL_ASHKARR_START_2026-09-12.rws`). If you want to know what is on a tile,
  ask the live world (`jawa/world_*` reads) or that save.
- **`world/ASHKARR_<STAGE>_<date>_{tiles,links,settlements}.csv`** — dated EXPORT
  bundles for diffing and review. Derived artifacts: trust one only after its
  fingerprint matches the live world, and never edit one expecting the world to change.
- **`src/RimMandrake/Utils/ashkarr_paint.py`** — the historical recipe that produced
  the first draft. Per the 08-22 adoption ruling it is NOT re-run; the map is edited
  directly, in place. `world/view/*.svg` and `*.png` are pictures of an export,
  rendered by `worldview.py` — never source material.

## Status

Written 2026-08-18 as the standing brief for the repaint; map-location and method
sections reconciled 2026-09-18 at the owner's worldmap docs pass. Authoring
continues directly in the live world; this file keeps the doctrine.
