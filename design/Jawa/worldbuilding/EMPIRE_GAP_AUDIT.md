...
| 6 | ~~`Jawa_AscendantHelix` wears the Empire's world icon.~~ ✅ **RULED, not a gap** (R16/R17, `src/RimUtinni/UtinniPatches/Defs/FactionDefs/JawaAscendantHelix.xml:19-21`): "The namers and the icon are the Empire set... the Helix is small, rich and formal, and the empire art is the only thing in the set that reads that way" — a deliberate shared-icon choice, landed via `FACTION_ART_SPEC_1` → `FACTION_ICONS_BESPOKE_1` (done, twelve bespoke icons shipped). ORIGINAL FINDING: two factions, one icon, on a map that is frozen | — | ⚠️ the icon is drawn live, the settlement names are not |
