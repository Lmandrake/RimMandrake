...
| `BMT_FungalForest` | ⛔ DISSOLVED 2026-09-06 (an underground def wearing 425 surface tiles) | raided into `the_rot.md` §7b (the spore-warfare kit, fungal materials, marsh fungi); tiles merge into the Rot (and the Wasteland at Sootreach sector 9) — `FUNGALFOREST_RAID_MERGE_1` |
...
| `the_webwork.md` | `AB_FeraliskInfestedJungle` ("the Webwork", owner-named; 172 tiles MEASURED, bimodal rain) | ✅ done 2026-09-06, two rounds — the drunk river; a biome-sized creature of shade; the Wyyyschokk/Feralisk canon; the light-moat; Shokkweave IS hyperweave, sole source; the three wars; NOT the Greentide, by inversion. **Owner+nest sitting 2026-09-23/24** (`webwork_owner_and_nest_2026-09-23.md` + flora/fauna rosters): invented `RM_Ollathrix` owner (Wyyyschokk becomes its Utinni skin), thrixweave, nest on every map, egg economy + campaign black market; all 16 rulings in §0/§6 |
...
| `the_scald.md` | `RUT_TheScald` (312 tiles, the substellar crater sea; FOUNDRY's waiting def now has its soul) | ✅ done 2026-09-07 — the boiler of the dayside; fouled heart, clean breath (the Contagion as the distillery's filter stage); crowncarpet mats, bottom-walker herds, silver shoals, bubble-sailors; the two-faith shore; salinity ruled fouled-not-toxic |
