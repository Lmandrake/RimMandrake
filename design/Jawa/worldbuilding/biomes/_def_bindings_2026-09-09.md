...
| `RUT_Contagion` | 179 | `the_contagion.md` | no | FROZEN 2026-09-25 (`CONTAGION_RM_MOD_BUILD_1`) at `src/RimUtinni/UtinniPatches/Defs/BiomeDefs/RUT_Contagion.xml` — carries the world unchanged until the terminal paint. Content (11 `wildAnimals` + 10 `wildPlants` rows) now lives in `mandrake.rm.contagion` (`src/RimMandrake/Contagion/Defs/BiomeDefs/RM_Contagion.xml`); zero Star Wars rows exist in the roster, so no `WildAnimals_Contagion.xml` patch was needed |
| `RUT_Webwork` | 161 | `the_webwork.md` | no | FROZEN 2026-09-25 (`WEBWORK_RM_MOD_BUILD_1`) at `src/RimUtinni/UtinniPatches/Defs/BiomeDefs/RUT_Webwork.xml` — carries the world unchanged until the terminal paint. The sibling `RM_Webwork` (`src/RimMandrake/Webwork/Defs/BiomeDefs/RM_Webwork_Biome.xml`) does NOT inherit this twin's content yet — it ships a vanilla-only placeholder pending `WEBWORK_FLORA_ROSTER_1`/`WEBWORK_FAUNA_ROSTER_1`; the frozen twin's 4 Star Wars `wildAnimals` rows ride `WildAnimals_Webwork.xml` as a Utinni patch onto `RM_Webwork` in the meantime |
