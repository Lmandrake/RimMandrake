...
**only where walkers have passed** (§8.2 `RM_CompMatTrail`) plus the shore margins the Deepfire
...
| S2 | **Walker + swirl + mat trail.** `RM_Ullum`, `RM_Tillik` races/kinds, `RM_CompMatTrail`, `RM_ThinkNode_SeekHeatEdge`, `RM_OrganicScaldNative` on both; comfy ranges; `<wildAnimals>` rows (ullum 0.15, tillik 0 — followers only) | `RM_CompMatTrail`, `RM_ThinkNode_SeekHeatEdge` (both small) | quicktest: spawn 3 ullum on the plain, `step_game_ticks` 5000: ≥1 `RM_DeepfireMat` exists on a cell an ullum occupied (compare `list_things` before/after); ullum positions cluster within 3 cells of a Hot/Plain boundary; damage one — it moves away, no attack job |
