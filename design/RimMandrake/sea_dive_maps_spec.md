...
1. **Plant growth-rate calculator throws on a tile-less map** when the biome has pasture-edible
   ...
   The Scald's Crowncarpet (`RM_Crowncarpet`, today still `RM_Crowncarpet` in TerminalBiomes)
...
- **Harvest** on the floor: Scald Crowncarpet (`RM_Crowncarpet` → `RM_CrowncarpetFresh`, the
...
**Deepfire's Crowncarpet.** The mat is `RM_Crowncarpet` (Deepfire spec §2.1 — the def moves out
of TerminalBiomes into LuminousPigment and the plant's common name is **Crowncarpet**, owner
card 16:16), harvesting `RM_CrowncarpetFresh` with its one-day clock. On the floor it grows
