# Ronto

**defName**: `Ronto` (third-party, mlie.starwarsanimalcollection, not vendored — see Watch out)

## Sourced text (Wookieepedia)
Rontos were four-legged saurian herbivores native to the desert world of
Tatooine — huge, imposing reptilian animals standing roughly 4-5 meters
(~16 ft) tall, with small eyes, pointed ears, and a beak-like mouth. Their
eyesight was poor, but large earflaps amplified sound and doubled as
sand-shielding for their weak eyes during sandstorms; smell was also
exceptional. They shed heat through their skin and the flap-like folds
framing their faces. Skittish and easily startled despite their size and
strength, when angered or challenged they reared onto their hind legs and
slammed down their forelegs as a weapon. They required large amounts of
water to survive despite being desert-adapted.

Easily domesticated, rontos became the favored pack animal and mount of the
Jawas, hauling cargo and Jawa passengers between Tatooine's trading posts and
settlements (Mos Espa, Mos Eisley). They were loyal, gentle, and strong once
tamed. Later galactic history: kept as menagerie beasts (e.g. by Grakkus the
Hutt), bred/wrangled at Freetown after the fall of the Empire, and — much
later — farmed for meat used in "Ronto Wraps" sold at Black Spire Outpost on
Batuu.

**Production note (important, corrects an assumption in this task's brief):**
per Wookieepedia's "Behind the scenes" section, the ronto was NOT in the
original 1977 theatrical cut of *A New Hope*. It was created for the 1997
*Star Wars Trilogy Special Edition* re-release, designed by artist TyRuben
Ellingson (who modeled concepts after elephants, rhinos, and dinosaurs).
Industrial Light & Magic built it by digitally modifying the Brachiosaurus
model from *Jurassic Park*; ILM animators nicknamed it "Bronto" during
production, and George Lucas dropped the "B" to get "Ronto." So the Mos
Eisley street shot below is a 1997 Special-Edition VFX addition composited
into the Tatooine scene, not a practical 1977 elephant-costume creature as
the task brief assumed.

## Visual brief
All four candidate images agree closely and reinforce each other — there is
no text/image conflict here, and no blue-grey coloration anywhere (that
"AI invented the wrong color" cautionary example in this task's instructions
evidently refers to a different creature, not this one).

- **Body plan**: long, gently S-curved sauropod-like neck rising to a
  triangular head; a distinct rhino/ceratopsian-style pointed nose horn or
  nasal ridge; a beaked, slightly downturned mouth; two large, wing-like
  skin-flap "ears" that fan out and back from the head/neck like a
  stylized cape or frill (these read almost as folded elephant ears, per
  Ellingson's stated elephant/rhino/dinosaur influences). Thick, heavily
  columnar legs (four), each ending in broad clawed/hoofed feet, and a
  short thick tail. Skin is deeply wrinkled/creased, especially at the neck
  and leg joints, with rounded wart-like or pebbly bumps scattered over the
  body (visible in both the infobox art and the concept sketches).
- **Color**: consistently a muted, dark olive-to-brownish-grey — NOT
  saturated brown, NOT blue-grey. The infobox illustration reads darkest
  (near-black-brown with ochre/tan wart highlights); the "Suns of Fortune"
  painted art and the film still both show a lighter, more olive-grey-green
  skin tone with darker striping/banding on the neck and back, and a paler,
  cream-to-tan underside/inner-ear color. Treat the **film still
  (`mos_eisley_street.jpg`) as the anchor reference** — it is the
  actual on-screen VFX creature all other art derives from/matches, and its
  olive-grey-green with darker banding is corroborated by the painted
  Suns of Fortune art (same hue family, just more saturated/painterly).
  The monochrome-ish infobox illustration is stylized/inked and should be
  read for silhouette and skin texture (wrinkles, warts, ear-flap shape),
  not for exact hue.
- **Scale/use**: every image with human/humanoid figures for scale (film
  still, concept sketches) confirms the "huge" text description — a ronto
  towers well over a standing human, consistent with the ~4-5m/16ft quoted
  height. The film still and the Suns of Fortune art both show tack/rigging
  — a bridle/rein around the head and neck, and a cargo saddle/howdah
  strapped across the shoulders — matching the sourced role as a Jawa pack
  and mount animal.
- No disagreement to flag beyond the production-history correction above:
  every image's silhouette, ear-flap shape, horn, beak, and body proportions
  are mutually consistent.

## Source URLs
- https://starwars.fandom.com/wiki/Ronto (page; fetched via API due to
  direct-fetch blocking)
- https://starwars.fandom.com/api.php?action=parse&page=Ronto&format=json&prop=wikitext
  (wikitext source used for all sourced text above)
- https://starwars.fandom.com/wiki/File:Ronto.png (infobox illustration)
- https://starwars.fandom.com/wiki/File:Enraged_Ronto-Suns-of-Fortune.jpg
  (painted art from *Suns of Fortune*, an enraged/reared ronto with tack)
- https://starwars.fandom.com/wiki/File:Ronto_in_Mos_Eisley.png (film still,
  *A New Hope* Special Edition, Mos Eisley street)
- https://starwars.fandom.com/wiki/File:Ronto_concepts.jpg (TyRuben
  Ellingson's original concept sketches for the 1997 Special Edition)
- https://steamcommunity.com/sharedfiles/filedetails/?id=3497316713
  (Star Wars Animal Collection (Continued), the mlie.starwarsanimalcollection
  Workshop page — checked for a donor Ronto screenshot, none identifiable)

Located the mod's Steam Workshop page — "Star Wars Animal Collection
(Continued)" by Mlie, id 3497316713
(https://steamcommunity.com/sharedfiles/filedetails/?id=3497316713),
confirmed by its own description to include a Ronto among 150+ Star Wars
creatures — but its preview-image gallery has no per-image captions, so no
individual screenshot could be identified as specifically showing the
Ronto (the gallery is a general grid of creatures, uncaptioned). No donor
sprite was obtainable this pass; all candidate images below are
Wookieepedia canon references, not mod screenshots.

## Candidate images
- `wookieepedia_infobox.jpg` — the species infobox illustration: inked/
  painted art, dark olive-brown, full body side-ish view, wrinkled skin with
  wart-like bumps, ear-flaps fanned back, nose horn, beak mouth, clawed feet.
  Source: File:Ronto.png (Wookieepedia infobox).
- `suns_of_fortune_enraged.jpg` — painted illustration of a rearing/enraged
  ronto with bridle and cargo saddle, olive-green-grey skin with darker
  neck banding, cream underside. Source: *Suns of Fortune* sourcebook art,
  File:Enraged_Ronto-Suns-of-Fortune.jpg.
- `mos_eisley_street.jpg` — **film still**, *A New Hope* (1997 Special
  Edition VFX shot), a saddled/rigged ronto walking a Mos Eisley street past
  R2-D2 and human-scale figures; the clearest canon color/scale anchor.
  Source: File:Ronto_in_Mos_Eisley.png.
- `ellingson_concept_art.jpg` — TyRuben Ellingson's original production
  concept sketches (rear 3/4 and front 3/4 views) with a human figure for
  scale, showing the elephant/rhino/dinosaur design influences and confirming
  silhouette and proportion. Source: File:Ronto_concepts.jpg.

## ruling
(empty — owner has not reviewed this creature yet)
