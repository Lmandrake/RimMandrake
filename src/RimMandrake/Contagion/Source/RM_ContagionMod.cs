...
namespace RimMandrake.Contagion
{
    ...
    // Burn/Bloom weather mechanic, the Unfinished's random-mutation spawner
    // and Contagion-touched are all unbuilt — biome_mod_architecture.md §5
    // step 1: "a ModSettings class with the master toggle only, no kit
    // mechanics exist yet to gate"), so the only control is the standard
    // worldgen-rarity slider every sibling biome mod ships.
    //
