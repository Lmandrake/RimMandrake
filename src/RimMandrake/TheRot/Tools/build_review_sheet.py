...
Builds ONE owner-facing review sheet over three groups:
  ...
  B) Rot fauna  — RUT_TheRot.xml's <wildAnimals> (MEASURED 9, not the 15 the build
     brief named — see the report) plus RM_Emberscythe, with the kin-mechanics
...
# appears exactly once in the whole flora set, on RM_MedicineFungal (an
...
FLORA_MESH = {
    "AB_Bryolux": 4, "RM_Dewshrooms": 9, "RM_FruitingBodies": 25,
    "RM_Wrinklecap": 4, "RM_Pusmelon": 4, "RM_RustPuff": 4,
    "RM_Sagecrust": 9, "RM_CrimsonCap": 4, "RM_VioletWimple": 4,
    "RM_MortalMorelPlant": 4, "RM_BlastpodShroom": 1, "RM_FurnaceCap": 4,
    "RM_PaleMoss": 9,
}
...
# RM_Emberscythe from its own def in this repo).
FAUNA_DRAWSIZE = {
...
RM_TEX = "src/RimUtinni/RotSporeKit/Textures/RotSporeKit/Things/Plant"
...
FLORA = [
    ...
    ("RM_Dewshrooms", "dewshrooms", "RotSporeKit (mandrake.rut.rotsporekit)", 0.5, "Dewshrooms light up to attract small prey, which becomes trapped in their sticky gills.", "RotSporeKit/Things/Plant/Seadew", "owned", f"{RM_TEX}/Seadew/BMT_SeadewA.png", ""),
    ("RM_FruitingBodies", "mold fruiting bodies", "RotSporeKit (mandrake.rut.rotsporekit)", 0.5, "The fruiting bodies of a mold that grows in caverns.", "RotSporeKit/Things/Plant/FruitingBodies", "owned", f"{RM_TEX}/FruitingBodies/FruitingBodyA.png", "texPath also reused as a placeholder by RM_FalseFruit (Guardian Groves, art queued)"),
    ("RM_Nuitae", "nuitae", "RotSporeKit (mandrake.rut.rotsporekit)", 0.5, "While its cap's exterior is largely dark in color, its underside glows brightly. It grows most often in water or", "RotSporeKit/Things/Plant/Nuitae", "owned", f"{RM_TEX}/Nuitae/Nuitae_A.png", ""),
    ("RM_Wrinklecap", "wrinklecap", "RotSporeKit (mandrake.rut.rotsporekit)", 0.5, "A subterranean fungus that grows in damp environments. It is often eaten by fungivores.", "RotSporeKit/Things/Plant/Wrinklecap", "owned", f"{RM_TEX}/Wrinklecap/Wrinkle1.png", ""),
    ("RM_Arpeau", "arpeau", "RotSporeKit (mandrake.rut.rotsporekit)", 0.4, "Fast-growing, but doesn't yield much usable material. Aquatic fungus.", "RotSporeKit/Things/Plant/Arpeau", "owned", f"{RM_TEX}/Arpeau/Arpeau_A.png", ""),
    ("RM_Nogtyl", "nogtyl", "RotSporeKit (mandrake.rut.rotsporekit)", 0.4, "A strong wood-like mushroom. Fast-growing, but doesn't yield much usable material. It can only be grown by people on marshy", "RotSporeKit/Things/Plant/Nogtyl", "owned", f"{RM_TEX}/Nogtyl/Nogtyl_A.png", ""),
    ("RM_FlakespireFungus", "flakespire fungus", "RotSporeKit (mandrake.rut.rotsporekit)", 0.3, "A strong purple mushroom tree. Slow-growing, but this woody fungus is very strong and attractive.", "RotSporeKit/Things/Plant/FlakespireFungus", "owned", f"{RM_TEX}/FlakespireFungus/Flakespirefungus_a.png", ""),
    ("RM_Pusmelon", "pusmelon", "RotSporeKit (mandrake.rut.rotsporekit)", 0.3, "A green, gourd-like fungus that produces sacs of foul-smelling liquid.", "RotSporeKit/Things/Plant/Pusmelon", "owned", f"{RM_TEX}/Pusmelon/BMT_PusmelonA.png", ""),
    ("RM_RustPuff", "rustpuff", "RotSporeKit (mandrake.rut.rotsporekit)", 0.3, "A large puffball mushroom with flaky, rust-colored spores.", "RotSporeKit/Things/Plant/RustPuff", "owned", f"{RM_TEX}/RustPuff/BMT_RustPuffA.png", ""),
    ("RM_Sagecrust", "sagecrust", "RotSporeKit (mandrake.rut.rotsporekit)", 0.3, "A tough, leafy lichen that can survive with minimal light.", "RotSporeKit/Things/Plant/Sagecrust", "owned", f"{RM_TEX}/Sagecrust/BMT_SagecrustA.png", ""),
    ("RM_BleedingTooth", "bleeding tooth", "RotSporeKit (mandrake.rut.rotsporekit)", 0.2, "A huge mushroom that secretes a red, blood-like fluid.", "RotSporeKit/Things/Plant/BleedingTooth", "owned", f"{RM_TEX}/BleedingTooth/BMT_BleedingToothA.png", ""),
    ("RM_Brightbell", "shinebell", "RotSporeKit (mandrake.rut.rotsporekit)", 0.2, "A small pretty mushroom that glows in the dark.", "RotSporeKit/Things/Plant/Brightbell", "owned", f"{RM_TEX}/Brightbell/Brightbell_A.png", 'the_rot.md names it "shinebell" (label), defName Brightbell — a fixed donor typo (BMT_Brightbells never existed), not a guess'),
    ("RM_CrimsonCap", "crimson cap", "RotSporeKit (mandrake.rut.rotsporekit)", 0.2, "A large crimson-capped mushroom. It is pretty enough to be used for decor but not much else.", "RotSporeKit/Things/Plant/CrimsonCap", "owned", f"{RM_TEX}/CrimsonCap/CrimsonCap_a.png", "texPath also reused as a placeholder by RM_FurnaceCap and RM_AgelessCap (both art queued)"),
    ("RM_GreyLady", "Grey Lady", "RotSporeKit (mandrake.rut.rotsporekit)", 0.2, "A grey fungus that grows cloth-like lace from its cap.", "RotSporeKit/Things/Plant/GreyLady/GreyLadyGrown", "owned", f"{RM_TEX}/GreyLady/GreyLadyGrown/GreyLadyGrownA.png", "texPath also reused as a placeholder by RM_RegenerantVeil (Guardian Groves, art queued)"),
    ("RM_Shinecap", "shine cap", "RotSporeKit (mandrake.rut.rotsporekit)", 0.2, "This large mushroom lives in symbiosis with a surprisingly tasty slime mold called Glimmerslime.", "RotSporeKit/Things/Plant/Shinecap/ShinecapGrown", "owned", f"{RM_TEX}/Shinecap/ShinecapGrown/ShinecapGrown_a.png", ""),
    ("RM_VioletWimple", "violet wimple", "RotSporeKit (mandrake.rut.rotsporekit)", 0.2, "A big violet-capped mushroom. While not very productive, it does produce edible fungus.", "RotSporeKit/Things/Plant/VioletWimple", "owned", f"{RM_TEX}/VioletWimple/BMT_VioletWimpleA.png", "texPath also reused as a placeholder by RM_EuphoricCrown (Guardian Groves, art queued)"),
    ("RM_MortalMorelPlant", "mortal morel", "RotSporeKit (mandrake.rut.rotsporekit)", 0.15, "A slow-growing mushroom which yields fungal medicine when harvested. Sowing and harvesting this fungus are both very labor-intensive because of", "RotSporeKit/Things/Plant/MortalMorel", "owned", f"{RM_TEX}/MortalMorel/CavernalMorel_A.png", "texPath folder says MortalMorel, on-disk files are CavernalMorel_* — pre-existing naming oddity, not fixed here"),
    ("RM_Skulltop", "skulltop", "RotSporeKit (mandrake.rut.rotsporekit)", 0.1, "An extremely deadly mushroom that steadily releases polluting toxic spores into the air. These spores rapidly produce toxic buildup in", "RotSporeKit/Things/Plant/Skulltop/Skulltop", "owned", f"{RM_TEX}/Skulltop/Skulltop/Skulltop_A.png", "the biome's lesser defender (AlphaBiomes gas-producer comp)"),
    ("RM_BlastpodShroom", "blastpod shroom", "RotSporeKit (mandrake.rut.rotsporekit)", 0.05, "A wild fungus prized for its dangerously volatile pods, which can be refined into chemfuel. It cannot be cultivated —", "RotSporeKit/Things/Plant/Boomshroom/BoomshroomGrown", "owned", f"{RM_TEX}/Boomshroom/BoomshroomGrown/BoomshroomGrown_A.png", "wild-only per owner ruling 2026-09-06, no sow tags"),

    # --- patch-added wild plants (separate files, never edit RUT_TheRot.xml directly) ---
    ("RM_PaleTree", "pale tree", "RotSporeKit (mandrake.rut.rotsporekit)", 0.01, "A bone-white tree that hums faintly at dusk, sacred to the wandering Wildsteam creed. Meditating in its shade for long", "Things/Plant/TreeAnima", "vanilla", str(RAW_VANILLA / "RM_PaleTree_vanilla.png"), "added by RotPaleTree_WildSpawn.xml (PatchOperationAdd), Royalty-gated; reuses vanilla TreeAnima verbatim (extracted TreeAnimaA from Data/Royalty/AssetBundles/resources_royalty, first alphabetically of A-E) — one of the 22 new-art jobs FAILED tonight on quota"),
    ("RM_AgelessCap", "ageless cap", "RotSporeKit (mandrake.rut.rotsporekit)", 0.03, "A pale, swollen mushroom rumored to hold back the years of anyone who tastes it right. It does not defend", "RotSporeKit/Things/Plant/CrimsonCap", "shared-placeholder", None, "added by RotGuardianGroves_WildSpawn.xml; texPath shared with RM_CrimsonCap and RM_FurnaceCap — one of the 22 new-art jobs FAILED tonight on quota"),
    ("RM_RegenerantVeil", "regenerant veil", "RotSporeKit (mandrake.rut.rotsporekit)", 0.03, "A translucent, slow-pulsing veil of fungal tissue, threaded straight into the grove's own mycelial network. Wound it and the", "RotSporeKit/Things/Plant/GreyLady/GreyLadyGrown", "shared-placeholder", None, "added by RotGuardianGroves_WildSpawn.xml; the alarm plant for ROT_GUARDIAN_GROVES_1 (currently 0 fauna responders — see Group B); texPath shared with RM_GreyLady — one of the 22 new-art jobs FAILED tonight on quota"),
    ("RM_EuphoricCrown", "euphoric crown", "RotSporeKit (mandrake.rut.rotsporekit)", 0.02, "A broad, faintly luminous cap that brews into the Rot's own pleasure tea. It keeps no defense of its own", "RotSporeKit/Things/Plant/VioletWimple", "shared-placeholder", None, "added by RotGuardianGroves_WildSpawn.xml; texPath shared with RM_VioletWimple — one of the 22 new-art jobs FAILED tonight on quota"),
    ("RM_FalseFruit", "false fruit", "RotSporeKit (mandrake.rut.rotsporekit)", 0.05, "Grown at the edge of a euphoric crown's own root mat, and shaped to match it. It looks exactly like", "RotSporeKit/Things/Plant/FruitingBodies", "shared-placeholder", None, "added by RotGuardianGroves_WildSpawn.xml, the lure ring; texPath shared with RM_FruitingBodies — one of the 22 new-art jobs FAILED tonight on quota"),

    # --- non-wild RotSporeKit plant ThingDefs (not in wildPlants at all) ---
    ("RM_DulcisPlant", "dulcis", "RotSporeKit (mandrake.rut.rotsporekit)", None, "An oddly sweet mushroom grown by cave dwellers in place of berries. It can survive in a wide range of", "RotSporeKit/Things/Plant/Dulcis/DulcisGrown", "owned", f"{RM_TEX}/Dulcis/DulcisGrown/DulcisGrown_a.png", "fungiponics' default grow target, NOT in wildPlants (sow-only, mustBeWildToSow paradoxically true — grows wild-style once sown); a same-day regen pass (dulcisgrown_a_v1 etc, PASS, not yet wired) reshot this already-owned art — see the report"),
    ("RM_FurnaceCap", "furnace cap", "RotSporeKit (mandrake.rut.rotsporekit)", None, "A squat, thick-walled mushroom that runs its metabolism hot enough to steam in cold air. Cut one open and the", "RotSporeKit/Things/Plant/CrimsonCap", "shared-placeholder", None, "ROT_WARM_MAT_1; NOT in wildPlants (not yet wild-spawned, ThingDef only); texPath shared with RM_CrimsonCap/RM_AgelessCap — one of the 22 new-art jobs FAILED tonight on quota"),
    ("RM_PaleMoss", "pale moss", "RotSporeKit (mandrake.rut.rotsporekit)", None, "A faintly glowing moss that only takes root beneath a pale tree, fed by whatever the tree is doing underground.", "Things/Plant/Grass_Anima", "vanilla", str(RAW_VANILLA / "RM_PaleMoss_vanilla.png"), "NOT in wildPlants (companion-spawn only, grows beneath RM_PaleTree); Royalty-gated; reuses vanilla Grass_Anima verbatim (extracted Grass_AnimaA from the base resources.assets, first alphabetically of A-C) — one of the 22 new-art jobs FAILED tonight on quota"),
]
...
FAUNA = [
    ...
    ("RM_Emberscythe", "emberscythe mantis", None, "NOT in RM_TheRot — a Pyrelands fire-follower; its def lives in UtinniPatches/Defs/ThingDefs_Races/RM_Emberscythe.xml, not in this kit", 1.3,
     "RotSporeKit (mandrake.rut.rotsporekit) — art is a vanilla Megascarab recolor placeholder, per its own header comment",
     str(RAW_VANILLA / "RM_Emberscythe_vanilla_south.png"),
...
# ============================================================================
GROUP_C = [
    ("twistingthornweed_v1_r2", "RM_TwistingThornweed (twisting thornweed)", "UtinniPatches — RM_PollutedFlora.xml (Cracked Lands / Poison Forest biomes, unrelated to the Rot or Lantern Deeps)",
    ...
    ("twitchingpuffer_grown_v1", "RM_TwitchingPuffer, grown stage", "LanternDeeps — RM_DeepFlora.xml",
    ...
    ("twitchingpuffer_harvested_v2", "RM_TwitchingPuffer, harvested/leafless stage", "LanternDeeps — RM_DeepFlora.xml",
    ...
    ("twitchingpuffer_immature_v1", "RM_TwitchingPuffer, immature stage", "LanternDeeps — RM_DeepFlora.xml",
    ...
    ("twitchingpuffer_tendrils_v1", "RM_PufferTendrils (harvested item)", "LanternDeeps — RM_LanternstoneItems.xml",
...
DULCIS_EXCLUDED = 4   # dulcis* jobs — the target defName (RM_DulcisPlant) is IN this sheet's Group A already
...
def main():
    ...
    cfg = {
        ...
        "briefHtml": (
            ...
            "were live. Plus RM_Emberscythe (ships in this kit but is Pyrelands, not Rot). Kin-mechanics "
        ...
        "invented": [
            ...
            "(RM_Glimmerslime, RM_MedicineFungal, RM_MoonlessSilk, RM_RawDulcis, RM_BlastSpore, "
            "RM_Proj_BlastSporeSac, RM_ChokingSpores).",
            ...
            "as measured (9 + RM_Emberscythe = 10 rows), not as briefed.",
            ...
            "art since 1.4. Resolved the same way as RM_PaleTree/PaleMoss/Emberscythe: UnityPy against the "
            ...
            "'donor'.",
            "RM_PaleTree/RM_PaleMoss (vanilla TreeAnima/Grass_Anima) and RM_Emberscythe (vanilla Megascarab "
            ...
            "also excluded from Group C because their target (RM_DulcisPlant) is a Group A row in THIS sheet.",
            ...
            "exactly once in the whole flora file set, on RM_MedicineFungal, an item not a plant) — but that's "
            ...
            "fixed one real number — RM_FlakespireFungus (RM_CaveTreeBase -> TreeBase) is 1.5~2.0, not the "
            "first pass's fallback of a bare 1.0 — and correctly re-labels the other 12 rows that inherit "
            "PlantBase's 0.3~1.0 (11 AlphaBiomes donor rows + RM_BleedingTooth) as 'vanilla default, "
            ...
            "its own PawnKindDef life-stage list (RSW_/AA_ from their own source files, RM_Emberscythe from "
