...
namespace RimMandrake.FluidCanals
{
	...
	public static class RM_ExcavationDepth
	{
		...
		public static TerrainDef DryTerrainFor(byte depth)
		{
			switch (depth)
			{
				case Shallow: return RimMandrakeFluidCanals_DefOf.RM_Channel_Empty;
				case Mid: return RimMandrakeFluidCanals_DefOf.RM_Channel_Mid;
				case Deep: return RimMandrakeFluidCanals_DefOf.RM_Channel_Deep;
				case Superdeep: return RimMandrakeFluidCanals_DefOf.RM_Channel_Superdeep;
				default: return null;
			}
		}
		...
		public static byte DepthOfDryTerrain(TerrainDef terrain)
		{
			...
			if (terrain == RimMandrakeFluidCanals_DefOf.RM_Channel_Empty) return Shallow;
			if (terrain == RimMandrakeFluidCanals_DefOf.RM_Channel_Mid) return Mid;
			if (terrain == RimMandrakeFluidCanals_DefOf.RM_Channel_Deep) return Deep;
			if (terrain == RimMandrakeFluidCanals_DefOf.RM_Channel_Superdeep) return Superdeep;
			return Surface;
		}
