	...
	public class RM_MapComponent_Excavation : MapComponent
	{
		...
		public FluidDef ActiveFluid
		{
			get
			{
				if (activeFluid == null)
				{
					activeFluid = RimMandrakeFluidCanals_DefOf.RM_Fluid_Water;
				}
				return activeFluid;
			}
		...
		private void RehydrateFromTerrainIfEmpty()
		{
			...
			if (recovered > 0)
			{
				Log.Message("[RimMandrake.FluidCanals] rehydrated " + recovered +
		...
		public void NotifyOverflowDestroyed(float fillUnits)
		{
			...
			if (Prefs.DevMode)
			{
				Log.Warning("[RimMandrake.FluidCanals] conservation exception: " +
		...
		public override void MapComponentTick()
		{
			...
			if (!RimMandrakeFluidCanalsSettings.depthEngineEnabled)
			{
				return;
			}
			...
			nextPulseTick = now + RimMandrakeFluidCanalsSettings.PulseIntervalTicks;
			DoPulse();
		}
		...
		private void ResolveComponent()
		{
			...
			int perCell = RimMandrakeFluidCanalsSettings.FlowPerPulse;
			...
			{
				Log.Warning("[RimMandrake.FluidCanals] conservation ledger does not balance: " +
		...
		private void ApplyFillTerrain(IntVec3 c, FluidDef fluid)
		{
			...
			if (want == null)
			{
				...
				// Flood_FluidCanal release path also writes the temp layer, and
