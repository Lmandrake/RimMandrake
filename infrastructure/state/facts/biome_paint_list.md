...
| `RUT_CrackedLands` | the Cracked Lands | `UtinniPatches` / `mandrake.rut.patches` | yes | terrain + plants + fauna present (162 lines) | Dew Horn, Cracklands, Damp, Dune Sea, Salt | 970 | **UNDECIDED** — twin pair with `RM_FloodedCanyon`; the DRAFT architecture doc (`biome_mod_architecture.md` §4d) says the owner has not yet confirmed the two describe the same place (§7 Q4) |
...
| `RUT_Slime` | the Slime (campaign twin) | `UtinniPatches` / `mandrake.rut.patches` | yes | terrain + plants + fauna present, 106 lines — thinner than its twin | Slough, Glass Reach, Nightspill, Chalk Marches | 96 | **UNDECIDED** — twin pair with `RM_GelatinousSlime`; unlike Greentide, no closed item was found ruling this pair, only the DRAFT doc's proposal (§4b) |
...
| `RM_FloodedCanyon` | flooded canyon | `src/RimMandrake/FloodedCanyon` / `mandrake.rm.floodedcanyon` | **NOT active** — absent from the live `ModsConfig.xml` (618 active mods, read 2026-09-20) | terrain + plants + fauna present (109 lines) | Cracked Lands (proposed pairing only, per DRAFT §4d) | 0 — the mod is inactive, so this def is absent from both the 618-mod def dump and the 2026-09-12 export | **UNDECIDED** — same open question as `RUT_CrackedLands` above: the DRAFT doc treats these as a twin pair but says plainly the owner has not confirmed it (§7 Q4) |
