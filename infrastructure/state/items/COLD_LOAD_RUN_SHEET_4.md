...
✅ **Deploys done 2026-09-21** while the game was down: `Pyrelands`, `PyrelandsMechanics`,
`FlowWorks`, `GelatinousSlime` — all `VERIFIED in sync`. ⚠️ Anything the agents still
running produce will need its own deploy before the load.

| # | entry | what it proves | state going in |
|---|---|---|---|
| 1 | `PYRELANDS_DEFNAME_RENAME_1` | `RM_Pyrelands` resolves; no red errors from the 3 renamed C# string literals | built at `84d42c63b`, ✅ **deployed and verified** |
| 2 | `OUR_MODS_DEPLOYED_NEVER_ACTIVATED_1` | GelatinousSlime loads | ✅ **ACTIVATED** — activeMods 619 → 620, parse-verified. Deployed too. Ready for the load |
| 3 | `TITANOSLIME_SLIME_BIOME_1` | the spec's seven §8 gates, with growth now **PERMANENT** (his one overturned default) | permanent growth **BUILT** at `8b9483b2e` (69/69 selftests, 0 warnings), ✅ **deployed and verified**. The Mod Settings toggle stays, unchecked by default |
