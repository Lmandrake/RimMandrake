...
## ✅ FIXED 2026-09-20 — the 21 droids were never a donor problem

Owner: *"Droid depot is our custom mod we are building."*

I had reported these as unportable because `neronix17.outerrim.droiddepot` is
absent from the mod list AND from the Workshop folder on disk. **That reasoning
was wrong.** `src/RimStarWars/Droidworks` (`mandrake.rsw.droidworks`, ACTIVE)
already **absorbed** the Outer Rim droids — `Defs/PawnKinds_OuterRim.xml`,
generated by `Source/gen_droidworks_defs.py` — and ships all seven as our own
`RSW_DW_OuterRim_*Droid` PawnKindDefs, which is exactly the def type
`<wildAnimals>` takes.

So nothing needed porting. The biome tables had simply never been repointed off
the donor names after we absorbed them.

**Done:** all 21 entries in `RUT_ExtremeDesert`, `RUT_Desert` and
`RUT_AridShrubland` repointed to `RSW_DW_OuterRim_*Droid` with
`MayRequire="mandrake.rsw.droidworks"`, commonalities unchanged.
**RE-MEASURED after the edit: 0 dead references remain in the desert family.**

🔑 **The lesson for the rest of this sweep:** a dead donor reference does not
mean the content is gone. Check whether we already own a replacement before
concluding anything is unportable — this project absorbs donors routinely
(`BMT_FAUNA_ABSORPTION_1`, `CAVERNS_PARITY_BUILD_1`, Droidworks), and the absorbed
def usually keeps a recognisable name.

⚠️ **Still open:** the 2 `BMT_GiantLeaf` entries (Greentide 1.0, FeverWood 0.8)
from the retired Biomes! Caverns. Check `BiomesTeamPort/` for an absorbed
equivalent before assuming those need new authoring.
